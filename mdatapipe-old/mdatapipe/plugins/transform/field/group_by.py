"""
Description: Produce group by statistics
"""
...
class Plugin(PipelinePlugin):
    ...
    def on_start(self):
        self.group_by = GroupBy(self, self.config)
...
class GroupBy(object):
    ...
    def add(self, item):
        ...
        group_key = '\n'.join(item[x] for x in self.config['fields'])
        ...
        for stats_field_name in self.stats_fields:
            item_value = int(item[stats_field_name])
    ...
    def send_group_results(self):
        ...
        for group_key, stats in self.aggregated_stats.items():
            ...
            for i, field_name in enumerate(self.config['fields']):
